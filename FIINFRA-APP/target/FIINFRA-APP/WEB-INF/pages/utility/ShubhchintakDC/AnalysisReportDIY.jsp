...
<c:otherwise>
<link href="<%=request.getContextPath()%>/css/${themeName}_output.css" rel="stylesheet">
</c:otherwise>
</c:choose>
...
<!--[if lt IE 9]>
            <script>
                ...
                document.createElement('fiagcaption');
            </script>
        <![endif]-->
...
function displayCashFlowBar() {
	...
	if(buId=='27830')
		...
		$('#pfordesclamerSPA').hide();
		}
	else if(buId=='27829')
		{
		$('#pfordesclamer').hide();
		$('#pfordesclamerSPA').show();
		}
	else{
...
//	$('body').hide();
	...
	familyMemberList='${userDetails}';
	familyMemberList=JSON.parse(familyMemberList);
	$.ajax({
        type: "GET",
        url : $('#contextPath').val()
...
//	            	alert('QuestionType='+QuestionType)
            	...
            	var ParentdId=$(this).attr('ParentQuestionId');
...
//		            			alert('J---'+j+'------html:--'+$('#riskProfile .left').html())
        	...
        	/*dfsd*/
//         	financial list started ajax cal
var questionDetails = '${questionDetails}';
console.log('questionDetails:----'+questionDetails)
questionDetails=JSON.parse(questionDetails);
//alert('questionDetails='+questionDetails)
//     		$.ajax({
// 				type : "POST",
// 				url : $('#context').val()
...
//						closePopupWebApp('centerLoadingImage');
				...
				var displayCashFlowBarData = '${displayCashFlowBarData}';
				displayCashFlowBarData=JSON.parse(displayCashFlowBarData);
// 				start display ash flow bar
// 				$.ajax({
// 					type : 'POST',
...
// 					success : function(result) {
						try{	
								...
								var yearsToRetirement=displayCashFlowBarData[0];
//					 				var periodArray = [ '1', '2', '3',
//					 						'4', '5'];				
								var outerArray = new Array();
								outerArray.push(innerArray);				
								//alert(result.length);
								for ( var i = 1; i < displayCashFlowBarData.length; i++) {
									innerArray = new Array();
									innerArray.push(i);
									innerArray.push(parseFloat(displayCashFlowBarData[i]));
									//alert(innerArray);
									outerArray.push(innerArray);
								}
...
//							alert(e);	
					...
					//	alert('barChartImageData='+barChartImageData);
						$.ajax({
							type : 'POST',
							url : $('#context').val()+'/utility/uploadGraphImage?dcInputPartyId='+dcInputPartyId,
...
//					alert($('#94018').val())
				...
				//console.log('>>>'+html);
//		 			alert('body='+html);
//						document.outputHtmlForm.submit();
				//alert('partyId==='+$('#dcInputPartyId').val());
				...
				$.ajax({
					type : 'POST',
					url : $('#context').val()+'/common/logout.htm',
					success : function(result) {
...
<input type="hidden" id="htmlContent" value="" name="htmlContent" /> 
			...
			value="<%=request.getContextPath()%>" /> 
			<input type="hidden" name="dcInputPartyId" id="dcInputPartyId" value="${output.partyId}">
				
			<input type="hidden" id="leadName" value="${output.opportunityName}">
...
<!-- 		<s:hidden type="hidden" id="profile" value="%{output.profile}" ></s:hidden> -->
		...
		<input type="hidden" id="section" value="${fpsection1}"/>
		<input type="hidden" id="section" value="${fpsection1}"/>
<body >
		...
		<div class="pf-op-container group">
         <div class="output_header"></div>
          ...
          <c:choose>
            ...
            <div class="clientlogo"><img  src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.buLogo}" style="height: 106px;width: 324px;" alt="Advisor Logo" /></div>
            </c:when>
            <c:otherwise>
  		 	<div class="clientlogo"><img  src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" style="height: 216px;width: 224px;" alt="Advisor Logo" /></div>
  		 	</c:otherwise>
  		 	</c:choose>
		  <div id="clientname">
 ...
 <h1> ${output.salutation} ${output.opportunityName}</h1>
  ...
  <div class="col-md-12" style="margin:20px 0 20px 12px;">
		   <div class="col-md-3">
               <div class="dcbox">
               <center>
                 <a><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/dc1op.png"></a>
               </center><br>
                      <h3> <a  title="Personal Information" alt="Personal Information" >Personal <br>
           ...
           <div class="col-md-3"> 
              <div class="dcbox"><br>          
              <center> 
                <a ><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/dc4op.png" ></a>
              </center> 
              <h3> <a  title="Risk Profile"alt="Risk Profile">Risk<br> Profile </a></h3></div>
              </div>
     		...
     		<div class="col-md-3">
               <div class="dcbox">
                <center>
                  <a ><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/dc3op.png"   ></a>
                </center><br>
                <h3><a  alt="Current Financial Status"  title="Current Financial Status"> 		Current <br>
          ...
          <div class="col-md-3">
              <div class="dcbox">
               <center> 
                 <a ><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/dc2op.png"></a>
               </center><br>
                 <h3><a  title="Future Financial Expectations" alt="Future Financial Expectations">
                 Future Financial <br>
...
<%--       		<li>Returns from investments are assumed at ${output.expectedReturnOnExistingInvestment}%.</li> --%>
            ...
            <li>If you have indicated a range value anywhere, the amount used for calculations is the average of the lower value and higher value in the range.</li>
      		  ...
      		  <c:choose >						
											...
											<li>Life insurance needs are calculated as per Human Life Value.</li>
										    </c:when>
										    <c:when test="${output.saving=='1'}">
										    <li>Life insurance needs are calculated as per Expense Replacement Method.</li>
										    </c:when>
										    <c:otherwise>
										   <li>Life insurance needs are calculated as per Income Replacement Method.</li>
										    </c:otherwise>
										 </c:choose>
...
<!--Welcome  page End ---->	 
<c:set var="pgcounter" value="1" />
<c:set var="theString" scope="session" value="${fpsection1}"/>
<!--1st page start -  Emergency Readiness --->		
<c:choose>
    <c:when test="${fn:contains(fpsection1, '39001')}">
...
<%--         			<div class="clogo"> <img style="width:40px;"  src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" /></div> --%>
       				<div style="font-size: 18px;">${output.partnerName}</div>
                        ...
                        <div class="output_base">Analysis Report for <strong>${output.salutation} ${output.opportunityName} ${sessionScope.Section} </strong></div>
...
<!--         following  emergencyQuestion div will daynamicaly load all selected question and options -->
            	...
            	<div id="mainright">       <img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/emergency.gif" />
        </div>  	 <br />
						<div class="pf-op-container group" >
     ...
     <c:choose>
    <c:when test="${fn:contains(fpsection1, '39003')}">
...
<%--                             <div class="clogo"> <img style="width:40px;"  src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" /></div> --%>
                            <div style="font-size: 18px;">${output.partnerName}</div>
                                ...
                                <div class="output_base">Analysis Report for <strong>${output.salutation} ${output.opportunityName}</strong></div>
                           <div class="output_header_head">Page ${pgcounter}
<%--                            <span style="margin:0px 0px 0 10px;"><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" width="19" height="19" /></span> --%>
                           </div>
                </div>
   <!--header Start--> 
       ...
       <div id="mainright">
       <img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/risk.gif" />
        </div>  	<br /> <br />
						<div class="pf-op-container group">
...
<!-- 								<img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/outputaxis/meter-3.png" alt="" style="with:192px; height:100px;"> -->
							...
							<div class="graphical-rep" class="current-sit" style="height:318px!important;">
			 		...
			 		<div class="suggestion" class="current-sit" style="height:318px!important;">
					<p class="heading">Our Suggestion</p>
...
<%--                        <span style="margin:0px 0px 0 10px;"><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" width="19" height="19" /></span> --%>
  ...
  <!--header Start--> 
    ...
    <div id="goalReadyQuestion">
        ...
        <div id="mainright">
       <img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/goal.gif" />
        </div>  	<br /> <br />
						<div class="pf-op-container group">
...
<!-- 								<img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/outputaxis/down1.png" alt="" class="Down"> -->
							...
							<div class="current-sit">
		                          <p class="heading">Goal of <span class="red" style="font-size: 19px;">${goalList.goalDescription}</span></p>
		                          <div class="small-pile" style="margin-top:70px"><p class="red">${goalList.startYear}<br />
		                            ...
		                            <c:choose >	
		                            <c:when test="${goalList.loanTakenOrNot=='0'}">
		                            ...
		                            <div class="small-pile"><p>You will need in <span class="red"><br />
			                          ${goalList.goalEndYear}</span> an amount of<br /> 
			                            <span class="red"> (Rs.${goalList.futureValueOfGoal}/-) </span>as downpayment for achieving your goal</p>
		                            </div>
		                             </c:otherwise>
		                            </c:choose>	
						  ...
						  <div class="suggestion">
								...
								<c:otherwise>
								<li>For the Downpayment please do a regular monthly investment of <span class="red"> Rs.${goalList.regularInvestment}/- </span>amount or one time investment of <span class="red">Rs.${goalList.goalPV}/- </span> amount.And the rest of requirment can be achived by taking loan. </li>
								</c:otherwise>
								</c:choose>
...
<%--                          <c:set var="counter" value="${counter+1}" /> --%>
						...
						<div id="disclaimer"><h2>Assumptions/Disclaimer</h2> 
                             Inflation is assumed at <span class="disclaimer_text">${goalList.description}%</span>. Investment need is calculated on the basis of <span class="disclaimer_text">${goalList.futureValueOfAssets}%</span> future returns.Assets are grown as per assumed growth rate of asset class. Assets are consumed in the order - Shares, mutual funds, deposits, bonds, gold and real estate in the order of liquidatability.All current investments
...
<!-- 								<img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/outputaxis/down1.png" alt="" class="up"> -->
        ...
        <div class="graphical-rep" style="height: 335px!important;">
							  <p class="heading">Achieving your Goal</p>
										...
										${goalList.startYear}</p>
									<p class="red question"
										style="position: relative;width: 30px;top: 120px;font-weight: bold;left:${(((goalList.percentGoalReachable-10)*20)/10)+132}px;">
										${goalList.goalEndYear}</p>
									<p class="red questionAnalysisR"
										style="position: relative;width: 30px;font-weight: bold;font-size: 16px;left:${(((goalList.percentGoalReachable-10)*20)/10)+158}px;top:20px;">
										${goalList.percentGoalReachable}%</p>
									<!-- 								<div style="position: relative;left: -5px;top:-41px;"> -->
									<input type="hidden" name="xyzzz" value="${(((goalList.percentGoalReachable-10)*20)/10)+195}">
									<img alt=""
										src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/outputaxis/arowdownyellow.png"
										style="bottom: 18px; width: 20px; margin-left: 141px;margin-top: -80px;">
...
<!-- 4th  page End -  Goal Readiness --->
 <c:choose>
    <c:when test="${fn:contains(fpsection1, '39008')}">
 ...
 <!--header end--> 
        <div id="mainleft">
           <!--1 start --> <div id="loanQuestions">
            	...
            	<div id="mainright"><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/loan.gif" alt="" /></div>  	
<br /> <br />
						<div class="pf-op-container group">
...
<c:set var="pgcounter" value="${pgcounter+1}" />	
 ...
 <c:choose>
    <c:when test="${fn:contains(fpsection1, '39006')}">
...
<%--                    <span style="margin:0px 0px 0 10px;"><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" width="19" height="19" /></span> --%>
        ...
        <div id="mainleft">
		 ...
		 <div id="retirementPlanQuestion">
            	...
            	<div id="mainright">
       <img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/retirement.gif" />
        </div>  	<br /> <br />
						<div class="pf-op-container group">
...
<!--6th  page End -  Retirement Planning ---> 
  ...
  <c:choose>
    <c:when test="${fn:contains(fpsection1, '39002')}">
...
<%--                    <span style="margin:0px 0px 0 10px;"><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" width="19" height="19" /></span> --%>
        ...
        <!--1 start -->
           <div id="cashflowQuestions">
           	   ...
           	   <div id="mainright">
       <img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/cash_flow.gif" />
        </div>  	<br /> <br />
						<div class="pf-op-container group">
							...
							<div class="bigboxtable" style="font-size:15px;">
                                    ...
                                    <tr>
                                      <td>Your Premium</td>
                                      <td ><span class="red">${output.salaryIncome}</span></td>
                                    </tr>
                                    <tr>
                                ...
                                TO MEET YOUR GOALS - ADDITIONAL INVESTMENT REQUIRED <span class="red"> Rs ${output.totalRegularInvestmentFromGoals} </span><br/>
								<br/>
...
<!--7th  page End -  Cash Flow --->
 ...
 <c:choose>
    <c:when test="${fn:contains(fpsection1, '39007')}">
...
<%--                    <span style="margin:0px 0px 0 10px;"><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" width="19" height="19" /></span> --%>
        ...
        <div id="mainleft">
 		<div id="successPlanQuestion">
            	...
            	<div id="mainright">
       <img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/images/${themeName}/succession.gif" />
        </div>  	<br /> <br />
						<div class="pf-op-container group">
...
<!--8th  page End -  Succession Readiness --->  
 ...
 <c:choose>
    <c:when test="${fn:contains(fpsection1, '39009')}">
...
<%--         			<div class="clogo"> <img style="width:40px;"  src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" /></div> --%>
       				<div style="font-size: 18px;">${output.partnerName}</div>
                        ...
                        <div class="output_base"> <strong>Your Risk Profile</strong></div>
                           <div class="output_header_head" style="margin-left:0px;">Page  ${pgcounter} 
<%--                            <span style="margin:0px 0px 0 10px;"><img src="<%=request.getRequestURL().toString().replace(request.getRequestURI().substring(0),request.getContextPath())%>/app/images${output.partnerLogo}" width="19" height="19" /></span> --%>
                           </div>
                       </div>
...
<input type="hidden" id="xmlString" value="">
