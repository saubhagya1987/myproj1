...
<head>
<style>
#assetOther label[for="currentValue"]{top: -44px;}
</style>	
<script>
$(function() {
...
$(document).ready(function(){
	...
	$( document ).ajaxComplete(function() {
		var x = document.getElementById("assetOther");
 ...
 $( ".ui-datepicker-trigger" ).wrap( "<div class='datepicker-trigger-parent fl'></div>" );
  $('.common_in_content').slimScroll();
     ...
     $("#assetTypeId").selectBoxIt();
     
     $("body").on("click", "#saveAdd", function() {
    	 if($('#assetOther').validate().form()){
    		 if(checkChangeData()){
    			 if(cntrSave==1){
    	 ...
    	 if($('#assetOther').validate().form()){
    		 if(checkChangeData()){
    			 if(cntrSave==1){
  	 				$("#saveAdd").addClass("inactiveLink");
  	    			$("#saveClose").addClass("inactiveLink");
  	    			$('#assetOther').attr('action', '${pageContext.request.contextPath}/assets/saveAndCloseAssets');
  		  			$('#assetOther').submit();
  	    			cntrSave++;
  	 			}
    		 }
...
selectBox1.selectOption('${assetOther.ownerPartyId}');

/* $("#assetTypeId").selectBoxIt().data('selectBox-selectBoxIt').add([{"value":"","text":"---Select---"}]);
$("#assetTypeId").selectBoxIt().data('selectBox-selectBoxIt').add(${frequencyList});
var selectBox2 = $("#assetTypeId").selectBoxIt().data('selectBox-selectBoxIt');			    
selectBox2.selectOption('${assetOther.assetTypeId}'); */


$("li").addClass("rel");

$("#assetOther").validate({ignore:'', rules:{currentValue:{min:1}}});
    ...
    <section class="content common_content">
      ...
      <section class="in_content common_in_content">
        <%-- <form:form id="assetCancel" method="POST" action="${pageContext.request.contextPath}/assets/assetDetails/${partyId}/0">
          ...
          <form:form id="assetOther" method="POST" action="${pageContext.request.contextPath}/assets/saveAssets">
          ...
          <input type="hidden" id="assetPage" name="assetPage" value="7"/>
          <input name="assetTypeId" type="hidden" value="48048"/>
          ...
          <div class="continer_parent">
            <div class="fd_box dynamic_box northspace6">
              <ul class="clearfix head_title first_level_heading reset">
                ...
                <!-- <li class="title_four label fl">Growth Rate(%)</li> -->
                <li class="title_five label fl">Current Value</li>
              ...
              <ul class="clearfix data_inputs first_level_inputs reset">
                ...
                <li class="input_two label fl">
                  <!-- <select id="assetTypeId" class="required" name="assetTypeId">
                  </select> -->
                  <input type="text" class="common_input required" name="assetName" value="${assetOther.assetName}"/>
                  <p style="color: #8D8D8D"><span style="color:#E90F0F">*</span> Note : Such Assets Can Include Vintage Cars, Antique Possessions, Patent Or Others</p>
                </li>
                <%-- <li class="input_four label fl">
                  <input type="text" class="common_input required number" name="interestRate" value="${assetOther.interestRate}"/>
                </li> --%>
                <li class="input_five label fl">
                  <input type="text" class="common_input required mynumber" id="currentValue" name="currentValue" value="${assetOther.currentBalance}"/>
                </li>                
              </ul>
              <c:choose>
      ...
      <section class="common_footer_actions">
        <div id="main_actions" class="clearfix">
          <ul class="reset clearfix fr btn_grup2">
           ...
           <li class="fl list"><a href="#" class="anch cancel" id="cancel" onclick="doAction('${pageContext.request.contextPath}/assets/assetDetails/${partyId}/0?assetPage=7');"><span class="txt">Cancel</span></a></li>
          </ul>
        </div>
