function jsonPopulateDropdownNew(url, data, dropdown, fieldName, selectedValue) {
	$(dropdown).empty();
...
function jsonPopulateDropdownForGoalsNew(url, data, dropdown, fieldName, selectedValue) {
	$(dropdown).empty();
	//$(dropdown).data("selectBox-selectBoxIt").add({ value: "", text: " -- Select --"});
	...
	$.ajax({
			...
			success : function(msg) {
				$(dropdown)
		         ...
		         .text('-- Select --'));
				$.each(msg, function(index, item) {					
					//$(dropdown).append( new Option(item.codeValue,item.codeValueId) );
					 $(dropdown)
			         .append($("<option></option>")
...
function createSliderNew(ctrlName, minVal, maxVal, stepBy, ctrlTextBox,setValue){
