		...
		while(rs.next()){
			...
			int key = partyAssestId > 0 ? partyAssestId:summaryId;
			 ...
			 if(asset ==null ){
				 ...
				 asset.setProductType(rs.getString("ProductType"));
//				 asset.setAssetClassId(rs.getInt("AssetClassID"));
//				 asset.setAssetClass(rs.getString("AssetClass"));
...
//				 asset.setAssestClassCAGR(rs.getInt("PerformanceCAGR"));
				 assests.add(asset);
				 if(partyAssestId >0){
					 map.put(key, asset);	 
				 }
