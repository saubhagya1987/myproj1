...
public class FiinfraAppService extends RestTemplate {
	...
	private void registerTypeRef() {
		...
		genericMapping.put(SyncData.class,
				new ParameterizedTypeReference<BaseResponse<SyncData>>() {
				});
	...
	public <T> BaseResponse<T>  postForObject(Services service, Object param,
	...
	public <T> BaseResponse<T> getBaseResponseForObject(Services service, BaseRequest request,
			Class<T> typeClass, String[] pathParams) {
		return  internalGetRequest(service, request,
				typeClass, pathParams);
	}
	...
	private <T> BaseResponse<T> internalRequest(Services service,
		...
		if(currentUser != null){
			...
			parameter.append("buId=").append(currentUser.getBuId()).append("&userId=").append(currentUser.getUserId())
		...
		if (HttpMethod.GET.equals(method) && param != null
			...
			parameter.append(((BaseRequest) param).buildQueryString());
		} else {
			requestEntity = new HttpEntity<Object>(param);
		}
		httpUrl.append("?");
		httpUrl.append(parameter);
