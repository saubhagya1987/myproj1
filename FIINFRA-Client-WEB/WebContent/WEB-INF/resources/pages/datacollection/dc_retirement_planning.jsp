...
<style>
.thumb_select_ul .categoery .newthumb {
	height: 100%;
	width: 100%;
	text-align: center;
	line-height: 97px;
		...
		<section class="content common_content">
			...
			<form class="form-horizontal" id="frmAssumption" name="frmAssumption">
					...
					<div class="common_ad_ass_selector selector_noline">
						...
						<div class="Advice_Assump_Menu">
							<ul class="clearfix reset Advice_Assump_Menu_In">
								<li class="clearfix"><label class="form_label">Estimated
										Life Expectancy</label>
									<div class="range_parent">
										<div class="scale clearfix">
											<div class="scale_range">
												<span class="low extrem">70</span><span class="high extrem">100</span>
											</div>
											<div id="98021"></div>
											<div class="counter_parent_new">
												<input type="text" id="98021_payment" name="98021_payment"
													class="range_counter Number required" /> <span class="unit">yrs</span>
											</div>
										</div>
									</div></li>
								<li class="clearfix"><label class="form_label">Set
										your default retirement age</label>
									<div class="range_parent">
										<div class="scale clearfix">
											<div class="scale_range">
												<span class="low extrem">40</span><span class="high extrem">70</span>
											</div>
											<div id="98022"></div>
											<div class="counter_parent_new">
												<input type="text" id="98022_payment" name="98022_payment"
													class="range_counter Number required" /> <span class="unit">yrs</span>
											</div>
										</div>
									</div></li>
								<li class="clearfix"><a class="cancel">CANCEL</a><a
									class="save">SAVE</a></li>
							</ul>
						</div>
					</div>
				</a>
				...
				<c:forEach items="${adviceAssumptions}" var="assumption">
					<c:choose>
						<c:when
							test="${assumption.assumptionCodeId eq '98021' or assumption.assumptionCodeId eq '98022'}">
							<script>
			...
			<form class="form-horizontal" id="frmEmergencyReadiness"
				name="frmEmergencyReadiness"
				action="<c:url value="/partner/datacollection/save" />"
				method="POST">
				<input type="hidden" name="partyId" id="partyId" value="${partyId}" />
				<input type="hidden" name="redirectUrl" id="redirectUrl"
					value="${redirectUrl}" /> <input type="hidden" name="initialData"
				...
				<section class="dc_in_content common_in_content clearfix"
					...
					<ul class="reset clearfix" id="emergency_readiness_ul">
						<x:forEach select="$output/Section/Question" var="item">
							...
							<c:set var="questionTypeMapTemp">
								<x:out select="$item/@AttributeCodeId" escapeXml="false" />:<x:out
									select="$item/@QuestionType" escapeXml="false" />
							</c:set>
							<c:set var="questionTypeMap"
								value="${questionTypeMap};${questionTypeMapTemp}" />
								...
								<div class="<x:out select="$item/@cssClass" />">
									...
									<x:when select="$item/@QuestionType = '85009'">
										<div class="ques_action clearfix">
											<div class="range_parent emer_confidence">
												<div class="fl scale clearfix">
													<div class="scale_range">
														<span class="low extrem"><x:out
																select="$item/Scale/Min_Value" /></span> <span
															class="high extrem"><x:out
													...
													<div class="counter_parent ">
														<input type="text" readonly
															id="<x:out select="$item/@AttributeCodeId" />"
															name="name_<x:out select="$item/@AttributeCodeId" />"
															value="" class="range_counter answer">
													</div>
													<script>
		                  			...
		                  			createSlider("#div_" + temp, <x:out select="$item/Scale/Min_Value" />, <x:out select="$item/Scale/Max_Value" />, <x:out select="$item/Scale/Increment" />, "#" + temp);
													...
													<x:forEach select="$item/Options/Option"
														var="optionElement">
														<c:set var="sliderOptionTemp">
															<x:out select="$optionElement/@AttributeCodeID"
																escapeXml="false" />:<x:out select="$optionElement"
																escapeXml="false" />
														</c:set>
														<c:set var="sliderOptionRange"
															value="${sliderOptionRange};${sliderOptionTemp}" />
													</x:forEach>

													<input type="hidden"
														id="sld_<x:out select="$item/@AttributeCodeId" />"
														name="<x:out select="$item/@AttributeCodeId" />"
														value="<c:out value="${sliderOptionRange}" />" />
												</div>
											</div>
										</div>
										<div class="clr"></div>
									...
									<x:when select="$item/@QuestionType = '85010'">
										<div class="ques_action clearfix">
			                  			...
			                  			createSlider("#div_" + temp, <x:out select="$item/Scale/Min_Value" />, <x:out select="$item/Scale/Max_Value" />, <x:out select="$item/Scale/Increment" />, "#" + temp);
													...
													<input type="hidden"
														id="sld_<x:out select="$item/@AttributeCodeId" />"
														name="<x:out select="$item/@AttributeCodeId" />" value="" />
												</div>
											</div>
										</div>
										<div class="clr"></div>
											...
											<div class="range_parent emer_confidence">
												<div class="fl scale clearfix">
													<div id="emergency_situation">
														<div class="toggle"
															id="div_<x:out select="$item/@AttributeCodeId" />"></div>
														<input type="hidden"
															id="<x:out select="$item/@AttributeCodeId" />"
															name="<x:out select="$item/@AttributeCodeId" />" value=""
															class="toggelCtrl answer" />
													</div>
													<script>
					             $('.toggle').toggles();
										...
										<div class="ques_action-checkbox">
											<ul>
												<x:forEach select="$item/Options/Option" var="optionElement">
													...
													<li class="${benfitTypeCss} fl">
														<div
															id="div_<x:out select="$optionElement/@AttributeCodeID" />"
															class="chekbox_parent">
															<span class="white_checkbox"></span>
															<div class="newthumb"
																style="background: url(${pageContext.request.contextPath}/resources/images/<x:out select='$optionElement/@ImgDefault' />) no-repeat center center;"></div>
															<input type="checkbox"
																id="<x:out select="$optionElement/@AttributeCodeID" />"
																name="<x:out select="$item/@AttributeCodeId" />"
																value="" class="default_checkbox answer" />
														</div> <span class="r-p-chkboxes1"><x:out
																select="$optionElement" /></span>
													</li>
												</x:forEach>
											</ul>
										</div>
									...
									<x:when select="$item/@QuestionType = '85016'">
										<div class="value-spinner northspace5 westspace3" id="test02">
											<input id="<x:out select="$item/@AttributeCodeId" />"
												name="<x:out select="$item/@AttributeCodeId" />"
												value="${defaultage}" class="spinner freeform answer" />
										</div>
										<script>
									...
									<x:when select="$item/@QuestionType = '85005'">
										<div class="right-side northspace4">
											<ul>
												<x:forEach select="$item/Options/Option" var="optionElement">
													<li><x:if select="$item/@ShowImage != 'FALSE'">
															<div class="icon">
																<ul
																	...
																	<li class="fl"><a href="#">
																			<div class="categoery job not_applicable">
																				<div class="newthumb"
																					style="background: url(${pageContext.request.contextPath}/resources/images/<x:out select='$optionElement/@ImgDefault' />) no-repeat center center;" /></div>
																			</div> <label class="label"><x:out
																					select="$optionElement" /></label>
																	</a></li>
																</ul>
															</div>
															<!-- end of "icon" -->
														...
														<div class="value-spinner">
															<input
																id="<x:out select="$optionElement/@AttributeCodeID" />"
																name="<x:out select="$item/@AttributeCodeId" />"
																value="0" class="spinner spinner85005 answer Number">
															<script>
										...
										<select id="select_<x:out select="$item/@AttributeCodeId" />"
											class="miniselect2">
											<option>K</option>
											<option>Lac</option>
											<option>Cr</option>
						...
						<div id="bottom">
			          	<div class="total-asset-value">
			            	<span>Total Assets Value</span><div id="sum_94148" class="total-value">0</div><span class="unit dynamic1">lac</span>
				...
				<section class="common_footer_actions">
					<div id="main_actions" class="clearfix">
						<ul class="reset clearfix fr btn_grup3">
							<li class="fl list"><a href="#" class="anch save"><span
									class="txt">Save</span></a></li>
							<li class="fl list"><a href="#"
								onclick="checkData('${pageContext.request.contextPath}/partner/datacollection');"
					...
					<a href="#"
						onclick="checkData('${pageContext.request.contextPath}${previousPageUrl}');"
						class="back navgation"><span class="txt">Back</span></a> <a
						href="#"
						onclick="doSubmitOnNext()"
			...
			$(".dynamic1").html("lac");
				...
				<c:set var="questionTypeMapArr"
					value="${fn:split(questionTypeMap, ';')}" />

				<!-- Iterate Already Set Data from questionDetails Map -->
				<c:forEach items="${questionTypeMapArr}"
					var="questionTypeMapArrVals" begin="0">
					<c:set var="questionTypeEle"
						value="${fn:split(questionTypeMapArrVals, ':')}" />
					<c:forEach items="${questionDetails}" var="questDetail">
						<c:set var="tempQuestionId"
							value="${fn:split(questDetail.questionId, '_')}" />
...
<div id="confirmDialog" class="common_custom_modal_aa">
	...
	<div class="modal_content">
		<div class="aa_up">
			<spring:message code="deleteRecord" />
		</div>
...
<!-- Wrapper ends-->
<script
	src="${pageContext.request.contextPath}/resources/js/plugin/jquery.ui.touch-punch.js"></script>
<script
	src="${pageContext.request.contextPath}/resources/js/common_unit_selector.js"></script>
...
$("#main_actions .save").click(function() {
	...
	if(initialData==inputData && $("#94044").val()!=currDefaultage){
