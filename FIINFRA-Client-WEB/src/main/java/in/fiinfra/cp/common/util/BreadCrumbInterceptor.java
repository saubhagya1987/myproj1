...
public class BreadCrumbInterceptor extends HandlerInterceptorAdapter {
	...
	private static final String BREAD_CRUMB = "currentBreadCrumb";
	...
	private void emptyCurrentBreadCrumb(HttpSession session) {
		session.setAttribute(BREAD_CRUMB, new LinkedList<BreadCrumbLink>());
	}
	...
	private void processAnnotation(HttpServletRequest request,
		...
		session.setAttribute(BREAD_CRUMB, currentBreadCrumb);
	}
	...
	private BreadCrumbLink getBreadCrumbLink(HttpServletRequest request,
		...
		if (breadCrumbObject != null) {
			...
			breadCrumbLink = new BreadCrumbLink(link.family(), link.label(),
					true, link.parent());
			//StringBuffer path = getFullPath(request);
			String fullURL =  request.getRequestURL().append("?").append(StringUtils.isEmpty(request.getQueryString())?"":request.getQueryString()).toString();
	...
	private StringBuffer getFullPath(HttpServletRequest request){
		return new StringBuffer("/"+(request.getRequestURI().substring(request.getContextPath().length()+1)));
	}
