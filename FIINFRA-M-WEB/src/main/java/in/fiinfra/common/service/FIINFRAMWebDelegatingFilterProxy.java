...
public class FIINFRAMWebDelegatingFilterProxy implements Filter {
	private static final Logger LOGGER = Logger.getLogger(FIINFRAMWebDelegatingFilterProxy.class);
	...
	public void doFilter(ServletRequest req, ServletResponse response,
		...
		String redirectUrl = baseUrl+"/FIINFRA-Welcome/common/login.htm";
		LOGGER.debug("In FIINFRAWebDelegatingFilterProxy " + request.getRequestURI());
		if (session.isNew() || session.getAttribute("SECURITY_TOKEN") == null) {
			...
			if ("abc".equals(token)) {
				...
				session.setAttribute("loginPageUrl", redirectUrl);
		...
		//  buid=userSession.getBuId();
		  LOGGER.debug("buid in logout="+buid);
