 ...
 * @param typeOfAlert
 *            e.g "information","confirmation","error","question" shows the alert message
...
//	var $jq_1_9_4=$;
	...
	$('#commonpopup-header-footer').addClass('hide');
	if(typeOfAlert=='confirmation')
		$('#commonpopup-header-title').html('<i class="fa fa-check bigger-130 eastspace2"></i>'+titleOfAlert);
	else if(typeOfAlert=='information')
		$('#commonpopup-header-title').html('<i class="fa fa-info  bigger-130 eastspace2"></i>'+titleOfAlert);
	else if(typeOfAlert=='error')
		$('#commonpopup-header-title').html('<i class="fa fa-exclamation bigger-130 eastspace2"></i>'+titleOfAlert);
	else if(typeOfAlert=='question'){
		$('#commonpopup-header-title').html('<i class="fa fa-question bigger-130 eastspace2"></i>'+titleOfAlert);
		$('#commonpopup-header-footer').removeClass('hide');
	}	
	$('#commonpopup-header-body').html(contentOfAlert);
	//$('#commonpopup-header-button-ok').attr('onclick','');
	$('#commonpopup-header').modal('show');
	if(typeOfAlert!='question'){
		setTimeout(function(){$('#commonpopup-header').modal('hide');}, 2000);
	}
	$('#commonpopup-header').on('hidden.bs.modal', function (e) {
		[ functionToBeCalled() ];
		});
	/*try {
...
function saveRtaFilePasswords() {
	var CAMS = $("#CAMS").val();
	var Karvy = $("#Karvy").val();
	var Sundaram = $("#Sundaram").val();
	var contactEmail = $("#alternateContactEmail").val();

	$.ajax({
		type : "POST",
		url : $('#contextPath').val()+'/partner/saveRtaFilePasswords?CAMS='+CAMS+'&Karvy='+Karvy+'&Sundaram='+Sundaram+'&contactEmail='+contactEmail,
		data: '',
		success : function(result) {
			closePasswordDialog();
			if($("#isRTAFilePasswordSet").val() == 1) {
				sendMail();
			} else {
				showAlertMessage("Success", "<br/>" + 
						 " Passwords updated Successfully", redirectFunction);
			}
...
function sendMail() {
	...
	var cc = '';
	var fromId = '';
	var to = $("#communicationId").val();
	var bcc = 'techsupport@fiinfra.in;praful.panchal@fiinfra.in';
	var partyId =$("#partyId").val();
	var deliveryChannelID = $("#deliveryChannelHeader").val();
	var useSignatureID = $("#useSignatureIDHeader").val();
	var partyIdString = $("#partyIdToSendString").val();
	var name=$("#partyName").val();
	var emailBody = "Dear Mr. "+ $.trim(name)+",<br><br>Your auto upload settings has been successfully completed. Somebody from Operations Team will get in touch with you to to set the Auto Forward rules.<br>";	
	var notifyTemplateXML='<root>';
	
	$.ajax({
		type : "POST",
		url : $('#contextPath').val()+'/partner/getEmailCommunicationDetails.htm',
		success : function(result) {
			from = result.emailCommunicatioName;
			cc= result.pstEmail + ';'+ result.mstEmail;
			fromId = result.emailCommunicationID;
			notifyTemplateXML += '<from>'
				+ from + '</from>';
		notifyTemplateXML += '<fromId>' + fromId
				+ '</fromId>';
		notifyTemplateXML += '<to>' + to + '</to>';
		notifyTemplateXML += '<cc>'+cc+ '</cc>';
		notifyTemplateXML += '<bcc>'+bcc+'</bcc>';
		notifyTemplateXML += '<phone></phone>';
		notifyTemplateXML += '<subject>Auto upload</subject>';
		notifyTemplateXML += '<body><![CDATA['+emailBody+']]></body>';
		notifyTemplateXML += '<replyTo>' + from + '</replyTo>';
		notifyTemplateXML += '<disclaimerText></disclaimerText>';
		notifyTemplateXML += '<dndText></dndText>';
		notifyTemplateXML += '<partyId>'+ partyId +'</partyId>';
		notifyTemplateXML+='</root>';
		$('#notifyTemplateXML').val(notifyTemplateXML);
		var url1 = $('#contextPath').val()+ '/ping/sendNotification.htm?notifyTemplateXML='+notifyTemplateXML+'&partyID='+partyId+'&deliveryChannelID='+deliveryChannelID+'&useSignatureID='+useSignatureID+'&partyIDString='+partyIdString;
		$.ajax({
			type : "POST",
			url : url1,
			data : '',
			success : function(result) {
				...
				if (recordCount != '0') {
					...
					showAlertMessage("Success", "<br/>" + 
							 " Thanks for registering for auto upload facility, we will get back to you at the earliest with the updated status.",
...
function showPasswordDialog(titleOfAlert, contentOfAlert, typeOfAlert,
		functionToBeCalled) {
	$.ajax({
		type : "GET",
		url : $('#contextPath').val()+'/partner/getRTAPasswords',
		data: '',
		success : function(result) {
			$("#CAMS").val(result.camsPassword);
			$("#Karvy").val(result.karvyPassword);
			$("#Sundaram").val(result.sundaramPassword);
			if(!(result.camsPassword == '' && result.karvyPasswor == '' && result.sundaramPassword =='')) {
				$("#isRTAFilePasswordSet").val(0);
			}
	...
	$('#commonpopup-header-footer').addClass('hide');
	/*if(typeOfAlert=='confirmation')
		$('#commonpopup-header-title1').html('<i class="fa fa-check bigger-130 eastspace2"></i>'+titleOfAlert);
	else if(typeOfAlert=='information')
		$('#commonpopup-header-title1').html('<i class="fa fa-info  bigger-130 eastspace2"></i>'+titleOfAlert);
	else if(typeOfAlert=='error')
		$('#commonpopup-header-title1').html('<i class="fa fa-exclamation bigger-130 eastspace2"></i>'+titleOfAlert);
	else if(typeOfAlert=='question'){*/
		$('#commonpopup-header-title1').html(titleOfAlert);
		$('#commonpopup-header-footer1').removeClass('hide');
	/*}	*/
	$('#commonpopup-header-body1').html(contentOfAlert);
	//$('#commonpopup-header-button-ok').attr('onclick','');
	$('#commonpopup-header1').modal('show');
	/*if(typeOfAlert!='question'){
		setTimeout(function(){$('#commonpopup-header1').modal('hide');}, 5000);
	}*/
	$('#commonpopup-header1').on('hidden.bs.modal', function (e) {
		[ functionToBeCalled() ];
		});
		},
	});
...
//	$('.ZebraDialog').css('z-index','999999');
	$('#commonpopup-header1').modal('hide');
}
/**
 * does nothing.
 */
...
function downloadAttachmentForRequestQ(reportTypeId,docId,partyId){
//	alert('RequestType----'+reportTypeId+'---Document ID----'+docId+'--PartyId--'+partyId);
	...
	else
	{
		...
		type :'POST',  
	     ...
	     url : $('#contextPath').val() +"/common/getAttachmentForRequestQ.htm?docId="+docId+'&partyId='+partyId+'&reportTypeId='+reportTypeId,	     	   
					success : function(result)
					{
						if(result=='success')
						{
							showAlertMessage1("Message", '<br/><span style="top:-5px;position:relative">Report Generation is in Process. Please Check After Sometime.</span>','information',
									doNothing1);
						}									
						else
						{
...
if (e.keyCode == 222 || e.keyCode == 192 || e.keyCode == 49 || e.keyCode == 50 || e.keyCode == 51 || e.keyCode == 52 || e.keyCode == 53 || e.keyCode == 54 || 
	...
	e.keyCode == 220 || e.keyCode == 219 || e.keyCode == 221 ) {
...
//				}
	...
	var partyIdForSendReport=localStorage.getItem('partyIdForSendReport');
	var buIdForSendReport=localStorage.getItem('buIdForSendReport');
	localStorage.setItem('partyIdForSendReport','0');
	localStorage.setItem('buIdForSendReport','0');
	
	var extraUrl='';
	if(partyIdForSendReport!='0'){
		extraUrl='&dcInputPartyId='+partyIdForSendReport+'&dcInputBuId='+buIdForSendReport;
	}
	
	$.ajax({
	     type : "POST",
	     url : $('#contextPath').val()+'/dc/submitDcInput.htm?isAnalysisReportGenration='+isAnalysisReportGenration+'&isActionPlanGeneration='+isActionPlanGeneration+'&analysisReportType='+analysisReportType+extraUrl,
	     data :  $('form').serialize(), 
	     success : function(result) {
	    	...
	    	// generateReportA(isAnalysisReportGenration,isActionPlanGeneration,isAnalysisReportAGeneration,isAnalysisReportBGeneration,isAnalysisReportCGeneration);
	    	 showAlertMessage1("Message",
   					"<br/>Reports will generate in 5 mins.",
...
function openPopupGenerateReport(partyId,buId)
{
	$('#popupGenerateReport').show();
	$('#blanket').show();
	var height = ($(window).height() / 2) - (parseInt($('#popupGenerateReport').css('height'))/2);
	$('#popupGenerateReport').css('top',height);
	var width = ($(window).width() / 2) - (parseInt($('#popupGenerateReport').css('width'))/2);
	$('#popupGenerateReport').css('margin-left', width);
	$('#popupGenerateReport').css('position','fixed');
	$('#popupGenerateReport').modal('show');
	localStorage.setItem('partyIdForSendReport',partyId);
	localStorage.setItem('buIdForSendReport',buId);
}

function openPopupMoveOpportunities()
{
	$('#popupMoveOpportunities').show();
	$('#blanket').show();
	var height = ($(window).height() / 2) - (parseInt($('#popupMoveOpportunities').css('height'))/2);
	$('#popupMoveOpportunities').css('top',height);
	var width = ($(window).width() / 2) - (parseInt($('#popupMoveOpportunities').css('width'))/2);
	$('#popupMoveOpportunities').css('margin-left', width);
	$('#popupMoveOpportunities').css('position','fixed');
	$('#popupMoveOpportunities').modal('show');
}

function closePopupMoveOpportunities()
{
	$('#popupMoveOpportunities').hide();
	$('#blanket').hide();
}

function closePopupGenerateReport()
{
	$('#popupGenerateReport').hide();
	$('#blanket').hide();
	}
...
function openSendReportPopup(partyId,name)
{
	$('#partyId').val(partyId);
	$('#convertNote').val('');
	$('#convertToContactType_error').addClass('hide');
	$('#convertNote_error').addClass('hide');
	getSendReport(partyId);
}
...
//	showAlertMessage1("Message", '<br/> Converting '+$('#partyName'+$('#contactPartyId').val()).html()+' from '+$('#currentContactType').html()+' to '+$('#convertToContactType option:selected').text()+'. Please confirm action.', "question",
	 showAlertMessage1("Message", '<br/> Converting from '+$('#currentContactType').html()+' to '+$('#convertToContactType option:selected').text()+'. Please confirm action.', "question",
			doNothing1);
...
function closeInsuranceMethodsPopup(){
	...
	$('#insuranceMethodsPopup').modal('hide');
	$('#blanket').hide();
}
...
//	}
	else if(cellvalue=='Execution Complete' || cellvalue=='Execution Open'){
...
function boldFormat(cellvalue, options, rowObject){
...
function validateInputForm1(formId){
	var isValid = true;
	$('#'+formId+' input').each(function (index, obj) {
		try{
		var validate_type = $(obj).attr("validate");
...
//
//$(document).ready(function() {
//	alert ('In ewwweeeewrwewrw.....')
//$(".noSpecialChar").on("keypress keyup blur",function (event) {
//$(this).val($(this).val().preg_replace(/[^a-zA-Z0-9-_\s]/gi,''));
//var value=$(this).val();
//
