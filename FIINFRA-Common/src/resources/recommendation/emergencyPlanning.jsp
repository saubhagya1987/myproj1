...
function setInactiveLink(ctrlName) {
	pieces=ctrlName.split("_");
	var id=parseInt(pieces[1])+1;
	var sliderId=""+pieces[1];
	...
	if($("#allocationPercent_"+sliderId).val()=="0"){						
		for(var i=1;i<totalRows;i++){
			...
			$(dropdownId).val(-1);
			$(dropdownId).addClass('inactiveLink');
			$("#slider_"+id+"_"+i).addClass('inactiveLink');
			$(companyDropdownId).addClass('inactiveLink');
			$(productDropDownId).addClass('inactiveLink');	
		}
	}
}
				...
				if($("#allocationPercent_"+sliderId).val()=="0"){						
        			...
        			for(var i=1;i<totalRows;i++){
        				var dropdownId="#productCategory_"+ id +"_"+i;	
    			  		var companyDropdownId="#companySelect_"+ id +"_"+i;	
    			  		var productDropDownId="#productSelect_"+ id +"_"+i;
    					$(dropdownId).removeClass('inactiveLink');
    					$("#slider_"+id+"_"+i).removeClass('inactiveLink');
    					$(companyDropdownId).removeClass('inactiveLink');
    					$(productDropDownId).removeClass('inactiveLink');
        			}        			
        		}        		
