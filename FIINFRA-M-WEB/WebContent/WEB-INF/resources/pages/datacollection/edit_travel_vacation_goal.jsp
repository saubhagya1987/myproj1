...
<div id="">
	...
	<div class="main-content">
     ...
     <form class="form-horizontal" id="travelVacationPlanId" name="travelVacationPlan" action="<c:url value="/futureFinancialDc/save-travel-vacation-goal" />" method="POST">
      ...
      <c:set var="attributeCount" value="${0}"></c:set>	
      	...
      	<x:parse xml="${sectionData}" var="output" />
      			...
      			<x:choose>
					...
					<x:when select="$item/@QuestionType = '85004'">
			         ...
			         <script type="text/javascript">			           
					   		...
					   		if(dropDownName=='#brand_4')
						   	{
				   			
					   			jsonPopulateDropdownNew("${pageContext.request.contextPath}/common/codevalues?codeTypeId=CountryForGoal", '', "#brand_"+${count.index}, "#brand_"+${count.index}, '${goalMap[attributeCodeId]}');
						   	}
					   		else if(dropDownName=='#brand_1')
						   	{
			       ...
			       <x:when select="$item/@QuestionType = '85009'">
							...
							<c:choose> <c:when test='${count.index == 6 }'>Number1</c:when><c:when test='${count.index != 6 }'>Number1</c:when></c:choose>" />
			    				...
			    				if(sliderVar=="#slider_9" || sliderVar=="#slider_10")
			    				{
			...
			src="${pageContext.request.contextPath}/resources/images/rupee-symbol_small.png" /> --%></label>
		                ...
		                <div class="col-sm-9 col-xs-10">
										<div class="input_counter_parent">
		                    <input id="spinner_${count.index}" name="spinner_${count.index}" value="${goalMap[attributeCodeId]}" class="input_counter answers required Number1">
		                  </div>
		                 </div>
		               </div>
	...
	<div class="row">
		...
		<div class="col-sm-10 col-xs-6">
			...
			<div class="col-sm-4 col-xs-4 test">
				<button
					class="btn btn-app btn-transparent btn-xs analysis " onclick="doValidate('travelVacationPlanId','${pageContext.request.contextPath}/analysis/travel/${partyId}/${goalId}')">
					<i class="ace-icon fa fa-search bigger-200"></i> Analysis
				</button>
				<button
					class="btn btn-app btn-transparent btn-xs suggeset " onclick="doValidate('travelVacationPlanId','${pageContext.request.contextPath}/analysis/suggestion/travel/${partyId}/${goalId}')">
					<i class="ace-icon fa fa-lightbulb-o bigger-200"></i> Suggestion
				</button>
				<button
					class="btn btn-app btn-transparent btn-xs action_plan " onclick="doValidate('travelVacationPlanId','${pageContext.request.contextPath}/actionplan/goal/travel-vacation/${partyId}/${goalId}')">
					<i class="ace-icon fa fa-hand-o-up bigger-200"></i> Action Plan
				</button>
			</div>
			<div class="col-sm-3 col-xs-4">
				<button class="btn btn-app btn-transparent btn-xs " onclick="uploadNote(${goalId},${partyId},'/futureFinancialDc/edit-travel-vacation-goall/${goalId}/${partyId}');">
					  <c:set var="docUrl" value="?partyId=${partyId}&url=/futureFinancialDc/edit-travel-vacation-goal/${goalId}/${partyId}"/>
					<i class="ace-icon fa fa-file-code-o bigger-200"></i> Notes
				</button>
				<button
					class="btn btn-app btn-transparent btn-xs dcAddNewDocumentLink"
					onclick="doValidate('travelVacationPlanId','${pageContext.request.contextPath}/partner/addDocument${docUrl}');" >
					  <c:set var="docUrl" value="?partyId=${partyId}&url=/futureFinancialDc/edit-travel-vacation-goal/${goalId}/${partyId}"/>
			...
			<div class="col-sm-4 col-xs-4">
				<button
					class="btn btn-app btn-transparent btn-xs saveEditTravelGoalLink save">
					<i class="ace-icon fa fa-save bigger-200"></i> Save
				</button>
				<button class="btn btn-app btn-transparent btn-xs"
					onclick="doValidate('otherGoalForm','${cancel}')">
...
$("#car_type").on('toggle', function (e, active) {
 ...
 // auto slider
     ...
     // on load change html
     var text6 = parseFloat( $("#sliderValue_9").val()) +  parseFloat($("#sliderValue_10").val());
    	 $("#slider_9").slider("option", "max", parseFloat($("#text__8").val()));
	     $("#slider_10").slider("option", "max", parseFloat($("#text__8").val()));
	     $("#sliderMaxValue_9").html($("#text__8").val());
	     $("#sliderMaxValue_10").html($("#text__8").val());
     ...
     $( document ).ajaxComplete(function() {
    		var x = document.getElementById("travelVacationPlanId");
  ...
  // css related class added
  //$(".wrapper").addClass('edit_travel_vacation_goal');
